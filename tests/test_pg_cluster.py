...
def test_standby_with_most_wal_gets_promoted2(cluster_context: ClusterContext):
    ...
    into promoting the wrong standby at first to make sure the correct standby
    is still promoted in the end """
    cluster_context.setup()
    cluster = cluster_context.cluster
    master = cluster.master
    ...
    assert scores_ok, (f"{stdby1.name} did not get the highest positive "
                       f"standby score")
    # give another standby the highest score
    cluster.standbies[:-1][0].ha_set_score(cluster.pgha_resource, "9999")
    master.vm_poweroff()
    if master.pg_version == "10":
...
def test_standby_with_most_wal_gets_promoted1(cluster_context: ClusterContext):
    """
    ...
    Check: stdby1 gets promoted directly
